...
userSelect.addEventListener('click', (e) => {
    selectList.classList.toggle('display-active');
})

// active-notify

const notifyBtn = $('.active-notify');
const notifyList = $('.active-notify-list');

notifyBtn.addEventListener('click', () => {
    notifyList.classList.toggle('display-active');
})

// active-setting

const settingBtn = $('.active-setting');
const settingList = $('.active-setting-list');

settingBtn.addEventListener('click', () => {
    settingList.classList.toggle('display-active');
})

// hidden selectList, active-notify, active-setting when user click outside on btn

document.addEventListener('click', (e) => {
    const userClick = userSelect.contains(e.target);
    const notifyClick = notifyBtn.contains(e.target);
    const settingClick = settingBtn.contains(e.target);

    if(!userClick) {
        selectList.classList.remove('display-active');
    }if(!notifyClick) {
        notifyList.classList.remove('display-active');
    }if(!settingClick) {
        settingList.classList.remove('display-active');
    }
})

// menu mobile

const menuOpen =  $('.navbar-icon');
...
menuOpen.addEventListener('click', () => {
    ...
    menuMobile.classList.remove('menuClose');
})
menuClose.addEventListener('click', () => {
    menuMobile.classList.add('menuClose');
...
const suggestItem = $('.suggest-list-item');
const inputSearch = $('.navbar-search-input');

function handelItem(suggestItem) {
    inputSearch.value = suggestItem.querySelector('p').innerHTML;
...
//  Delete selected suggest item

const delItem = $('.del-selected-icon');
...
const searchKeywords = $('.search-keyword');
const keywordList = $('.keyword-list');
    inputSearch.onkeyup = (e) => {
        ...
        let dataArray = [];
    if(checkData) {
        dataArray = listKeywords.filter((data) => {
        ...
        keywordList.classList.add('list-active');
        showData(dataArray);
        let liItem = keywordList.querySelectorAll("li");
        for( let i = 0; i < liItem.length; i++) {
                liItem[i].addEventListener('click', () => {
                inputSearch.value = liItem[i].querySelector('p').innerHTML;
                keywordList.classList.remove('list-active');
                delItem.style.display = 'block';
            })
        }
    }else {
        keywordList.classList.remove('active')
    }
}
...
// Hidden form when user clicks on outside of search input
document.addEventListener('click', (e) => {
    const isClickInputSearch = inputSearch.contains(e.target);
    const delIconClick = delItem.contains(e.target);
    if(!isClickInputSearch && !delIconClick) {
        searchSuggest.style.display = 'none';
        searchKeywords.style.display = 'none'
    }
})   
